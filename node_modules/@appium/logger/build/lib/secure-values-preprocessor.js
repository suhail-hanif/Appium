...
class SecureValuesPreprocessor {
    ...
    parseRule(rule) {
        ...
        if (typeof rule === 'string') {
            ...
            pattern = `\\b${(0, utils_1.escapeRegExp)(rule)}\\b`;
        }
        else if ((0, utils_1.isPlainObject)(rule)) {
            if (isLogFilterRegex(rule)) {
                ...
                pattern = `\\b${(0, utils_1.escapeRegExp)(rule.text)}\\b`;
            }
            if (!pattern) {
    ...
    async loadRules(filters) {
        ...
        for (const source of Array.isArray(filters) ? filters : [filters]) {
            if ((0, utils_1.isPlainObject)(source)) {
