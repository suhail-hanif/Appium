...
async function checkMissingPaths(opts, group, argv) {
    ...
    const pathsToCheck = argsToCheck.map((id) => ({
        id,
        path: String(argv[id]), // this must be a string per the above check
    }));
    log.debug('Checking for existence of %s: %s', support_1.util.pluralize('path', pathsToCheck.length), pathsToCheck.map(({ path }) => path));
    const missingPaths = await filterMissing(pathsToCheck);
    if (missingPaths.length) {
        return missingPaths
            .map(({ id, path }) => `Default or specified path via --${(0, utils_1.kebabCase)(id)} (${path}) does not exist`)
            .join('\n');
    }
    return true;
}
