...
export async function getChromeVersionForAutodetection() {
    ...
    let chromeVersion = await resolveChromeVersionByProbingWebviews(this);
...
function tryCoerceVersionFromCdpDetails(ctx) {
    if (ctx.details?.info) {
...
async function resolveChromeVersionForWebviewShell(ctx) {
    if (!ctx.adb) {
        return null;
    }
...
async function remapLegacyWebviewBundleId(ctx) {
    if (!ctx.adb) {
        return;
    }
 ...
 */
async function resolveChromeVersionByProbingWebviews(ctx) {
    if (ctx.bundleId) {
        return null;
    }
    ctx.bundleId = CHROME_BUNDLE_ID;
